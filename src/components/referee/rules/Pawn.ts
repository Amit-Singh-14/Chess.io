...
export const pawnMove = (initialPosition: Position, desiredPosition: Position, team: TeamType, boardState: Piece[]): boolean => {
  ...
  export const getPossiblePawnMoves = (pawn: Piece, boardState: Piece[]): Position[] => {
    ...
    const normalMove = 
    new Position(pawn.position.x,pawn.position.y + pawnDirection);

  const specialMove = 
  new Position( normalMove.x, normalMove.y + pawnDirection)
  const upperLeftAttack = 
  new Position(pawn.position.x - 1, pawn.position.y + pawnDirection)

  const upperRightAttack = 
  new Position( pawn.position.x + 1, pawn.position.y + pawnDirection )

  const leftPosition = 
    new Position(pawn.position.x - 1,pawn.position.y );
  const rightPosition = 
